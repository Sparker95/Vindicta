...
_o setVariable ["l_name", _name, false]; //The name of this location. Mainly for debug reasons.
...
_o setVariable ["l_boundingRadius", 5]; //This will be recalculated later with setBorderXXX functions
/*
Border data is:
	radius(number) - if the border type is circle
	[a, b, direction] - if the border type is a rectangle
*/
_o setVariable ["l_borderData", 5]; //If the border is circle, it only needs a radius
