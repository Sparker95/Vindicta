...
_this spawn {
	...
	private _fnc_checkEvents = {
		params ["_unit_1","_unit_2","_events","_conversation_args"];
	...
	while{true}do{
		...
		{	
			private _dataSet = _x;
			{
				...
				if (tolower _node_id_x isEqualTo tolower _node_id)exitWith{
					...
					if(isNil "_node_array" || {!(_node_array isEqualType [])})exitWith{format["ERROR NODE DOESNT RETURN A ARRAY:%1",_node_id] call _fnc_error};
		...
		//if((count _sentences + count _question) == 0)exitWith{			// We want to have logic nodes without sentences or questions
		...
		[_unit_1,_unit_2,_events,_conversation_args] call _fnc_checkEvents;
		...
		if(count _question > 0)then{
			...
			if(_listener in Allplayers)then{
				...
				_listener setVariable ["pr0_dialogue_answer_index",nil];
				private _timer = time + FLOAT_MAX_WAIT_FOR_ANSWER;
				waitUntil{
					sleep 0.1;
					[_unit_1,_unit_2,_events,_conversation_args] call _fnc_checkEvents;
					if(time > _timer)exitWith{true};
					!isNil {_listener getVariable "pr0_dialogue_answer_index"};
				};
				private _answer_index = _listener getVariable ["pr0_dialogue_answer_index", -TYPE_EVENT_OUT_OF_TIME];
