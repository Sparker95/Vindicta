...
params [P_THISOBJECT, ["_building", objNull, [objNull]]];
...
if(_index != -1) then {
	...
	//Add every position from the array to the spawn positions array
	{
		...
		if(count _bp == 2) then { //This position is defined by building position ID and direction
			...
			private _args = [T_PL_HMG_GMG_high, [GROUP_TYPE_IDLE, GROUP_TYPE_VEH_STATIC], _position, _bdir + (_bp select 1), _building]; // [P_ARRAY("_unitTypes"), P_ARRAY("_groupTypes"), P_ARRAY("_pos"), P_NUMBER("_dir"), ["_building", objNull, [objNull]] ];
			...
			private _args = [T_PL_HMG_GMG_high, [GROUP_TYPE_IDLE, GROUP_TYPE_VEH_STATIC], _position, _bdir + (_bp select 3), _building]; // [P_ARRAY("_unitTypes"), P_ARRAY("_groupTypes"), P_ARRAY("_pos"), P_NUMBER("_dir"), ["_building", objNull, [objNull]] ];
			T_CALLM("addSpawnPos", _args);
...
if (_index != -1) then {
	private _bps = location_bp_cargo_medium select _index;
	{
		...
		if(count _bp >= 3) then { //This position is defined by offset in cylindrical coordinates
			_position = (getPosATL _building) vectorAdd [(_bp select 0)*(sin (_bdir + (_bp select 1))), (_bp select 0)*(cos (_bdir + (_bp select 1))), _bp select 2];
			private _args = [T_PL_cargo_small_medium, [GROUP_TYPE_IDLE], _position, _bdir + (_bp select 3), _building]; // [P_ARRAY("_unitTypes"), P_ARRAY("_groupTypes"), P_ARRAY("_pos"), P_NUMBER("_dir"), ["_building", objNull, [objNull]] ];
			T_CALLM("addSpawnPos", _args);
...
if(count _bps > 0) then {
	//Add every position from the array to the spawn positions array
	{
		...
		if(count _bp == 2) then {//This position is defined by building position ID and direction
			_position = _building buildingPos (_bp select 0);
			private _args = [T_PL_inf_main, [GROUP_TYPE_BUILDING_SENTRY], _position, _bdir + (_bp select 1), _building]; // [P_ARRAY("_unitTypes"), P_ARRAY("_groupTypes"), P_ARRAY("_pos"), P_NUMBER("_dir"), ["_building", objNull, [objNull]] ];
			T_CALLM("addSpawnPos", _args);
		} else { //This position is defined by offset in cylindrical coordinates
			_position = (getPosATL _building) vectorAdd [(_bp select 0)*(sin (_bdir + (_bp select 1))), (_bp select 0)*(cos (_bdir + (_bp select 1))), _bp select 2];
			private _args = [T_PL_inf_main, [GROUP_TYPE_BUILDING_SENTRY], _position, _bdir + (_bp select 3), _building]; // [P_ARRAY("_unitTypes"), P_ARRAY("_groupTypes"), P_ARRAY("_pos"), P_NUMBER("_dir"), ["_building", objNull, [objNull]] ];
			T_CALLM("addSpawnPos", _args);
		};
