...
CLASS("ActionGarrisonDefendPassive", "ActionGarrisonDefend")
	...
	METHOD("terminate") {
		...
		{ // foreach _groups
			...
			if (_groupAI != NULL_OBJECT) then {
				CALLM2(_groupAI, "postMethodAsync", "deleteExternalGoal", ["goalGroupRelax"]);
				CALLM2(_groupAI, "postMethodAsync", "deleteExternalGoal", ["goalGroupPatrol"]);
				CALLM2(_groupAI, "postMethodAsync", "deleteExternalGoal", ["goalGroupGetInBuilding"]);
			};
		} forEach _groups;
	...
	METHOD("handleGroupsAdded") {
		params [P_THISOBJECT, P_OOP_OBJECT("_groups")];
