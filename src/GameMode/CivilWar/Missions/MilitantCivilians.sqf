...
vin_fnc_CivilianJoinPlayer = {
	...
	if(count units _grp < 5) then {
		...
		[_target, "You are too many already, we must be inconspicuous!"] call vin_fnc_dialogue_createSentence;
	};
};

vin_fnc_generateRandomPath = {
...
vin_fnc_militantFindWeapon = {
	...
	if(count _sourcesWithWeapons > 0) then {
		...
		[_this, _src] spawn {
			params ["_civ", "_src"];
			if([_civ, _src, 3] call vin_fnc_doMoveTimeout) then {
				doStop _civ;
				...
				if(count _weapons > 0) then {
					...
					sleep 3;
					_civ call vin_fnc_selectPrimaryWeapon;
					sleep 3;
...
vin_fnc_givePlayerIntel = {
	...
	if(count _players > 0) then {
		...
		[_civ, _tgt, _intel] spawn {
			...
			if([_civ, _tgt, 3] call vin_fnc_doMoveTimeout) then {
				doStop _civ;
				_civ lookAt _tgt;
		...
		// Alive civs can do some things
		{
			...
			switch true do {
				...
				case ((_instability > 0.35 || _ledByPlayer)
					&& {primaryWeapon _civ == ""}
					&& {_civ call vin_fnc_militantFindWeapon}): {
					// continue
				};

				// Look for a vest and helmet
				case ((_instability > 0.5 || _ledByPlayer)
					&& {_civ call vin_fnc_militantFindVest}): {
					// continue
				};
				...
				case (_instability > 0.65 
					&& !_ledByPlayer
					&& {[_civ, _activeCivs] call vin_fnc_findLeader}): {
					// continue
				};

				case (GAME_TIME >= T_GETV("nextInformant") 
					&& !_ledByPlayer
					&& {[_civ, T_GETV("newIntel")] call vin_fnc_givePlayerIntel}): {
					// continue
					#ifdef MILITANT_CIVILIANS_TESTING
				...
				case (!_ledByPlayer 
					...
					&& {[_pos, _radius * 0.5, group _civ] call vin_fnc_generateRandomPath}): {
					// continue;
				};
			};
