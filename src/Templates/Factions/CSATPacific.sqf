...
Updated: March 2020 by Marvis and Spectrum.
*/
...
_array set [T_NAME, "tCSAT_Pacific"]; // 														Template name + variable (not displayed)
_array set [T_DESCRIPTION, "Standard Canton Protocol Strategic Alliance Treaty Pacific from base game."]; // 	Template display description
_array set [T_DISPLAY_NAME, "Arma 3 CSAT Pacific"]; // 											Template display name
...
_inf set [T_INF_default,  ["O_T_Soldier_F"]];		//Default infantry if nothing is found

_inf set [T_INF_SL, ["O_T_Soldier_SL_F"]]; // = 1
_inf set [T_INF_TL, ["O_T_Soldier_TL_F"]]; // = 2
_inf set [T_INF_officer, ["O_T_Officer_F"]]; // = 3
_inf set [T_INF_GL, ["O_T_Soldier_GL_F"]]; // = 4
_inf set [T_INF_rifleman, ["O_T_Soldier_F"]]; // = 5
_inf set [T_INF_marksman, ["O_T_Soldier_M_F"]]; // = 6
_inf set [T_INF_sniper, ["O_T_Sniper_F"]]; // = 7
_inf set [T_INF_spotter, ["O_T_Spotter_F"]]; // = 8
_inf set [T_INF_exp, ["O_T_Soldier_Exp_F", "O_T_soldier_mine_F"]]; // = 9
_inf set [T_INF_ammo, ["O_T_Soldier_A_F"]]; // = 10
_inf set [T_INF_LAT, ["O_T_Soldier_LAT_F"]]; // = 11
_inf set [T_INF_AT, ["O_T_Soldier_HAT_F", 3, "O_T_Soldier_AT_F", 1]]; // = 12
_inf set [T_INF_AA, ["O_T_Soldier_AA_F"]]; // = 13
_inf set [T_INF_LMG, ["O_T_Soldier_AR_F"]]; // = 14
_inf set [T_INF_HMG, ["O_T_Soldier_AR_F"]]; // = 15
_inf set [T_INF_medic, ["O_T_Medic_F"]]; // = 16
_inf set [T_INF_engineer, ["O_T_Engineer_F", "O_T_Soldier_Repair_F"]]; // = 17 
_inf set [T_INF_crew, ["O_T_Crew_F"]]; // = 18
_inf set [T_INF_crew_heli, ["O_T_Helicrew_F"]]; // = 19
_inf set [T_INF_pilot, ["O_T_Pilot_F"]]; // = 20
_inf set [T_INF_pilot_heli, ["O_T_Helipilot_F"]]; // = 21
//_inf set [T_INF_survivor, []]; // = 22
//_inf set [T_INF_unarmed, []]; // = 23
/* Recon unit classes */
_inf set [T_INF_recon_TL, ["O_T_Recon_TL_F"]]; // = 24
_inf set [T_INF_recon_rifleman, ["O_T_Recon_F"]]; // = 25
_inf set [T_INF_recon_medic, ["O_T_Recon_Medic_F"]]; // = 26
_inf set [T_INF_recon_exp, ["O_T_Recon_Exp_F"]]; // = 27
_inf set [T_INF_recon_LAT, ["O_T_Recon_LAT_F"]]; // = 28
_inf set [T_INF_recon_marksman, ["O_T_Recon_M_F", 1]]; // = 29
_inf set [T_INF_recon_JTAC, ["O_T_Recon_JTAC_F"]]; // = 30
/* Diver unit classes */
...
_veh set [T_VEH_DEFAULT, ["O_T_LSV_02_unarmed_F"]]; // = 0 Default if nothing found

_veh set [T_VEH_car_unarmed, ["O_T_LSV_02_unarmed_F"]]; // = 1 – REQUIRED
_veh set [T_VEH_car_armed, ["O_T_LSV_02_armed_F", "O_T_LSV_02_AT_F"]]; // = 2
_veh set [T_VEH_MRAP_unarmed, ["O_T_MRAP_02_ghex_F"]]; // = 3 – REQUIRED
_veh set [T_VEH_MRAP_HMG, ["O_T_MRAP_02_hmg_ghex_F"]]; // = 4 – REQUIRED
_veh set [T_VEH_MRAP_GMG, ["O_T_MRAP_02_gmg_ghex_F"]]; // = 5 – REQUIRED
_veh set [T_VEH_IFV, ["O_T_APC_Tracked_02_cannon_ghex_F_1"]]; // = 6 – REQUIRED
_veh set [T_VEH_APC, ["O_T_APC_Wheeled_02_rcws_v2_ghex_F_1"]]; // = 7 – REQUIRED
_veh set [T_VEH_MBT, ["O_T_MBT_02_cannon_ghex_F_1", "O_T_MBT_02_cannon_ghex_F_2", "O_T_MBT_04_cannon_F_1", "O_T_MBT_04_cannon_F_2", "O_T_MBT_04_command_F_1", "O_T_MBT_04_command_F_2"]]; // = 8 – REQUIRED
_veh set [T_VEH_MRLS, ["O_T_MBT_02_arty_ghex_F"]]; // = 9
_veh set [T_VEH_SPA, ["O_T_MBT_02_arty_ghex_F"]]; // = 10
_veh set [T_VEH_SPAA, ["O_T_APC_Tracked_02_AA_ghex_F"]]; // = 11
...
_veh set [T_VEH_heli_attack, ["O_Heli_Light_02_dynamicLoadout_F"]]; // = 23
...
_veh set [T_VEH_boat_unarmed, ["O_T_Boat_Transport_01_F"]]; // = 29
_veh set [T_VEH_boat_armed, ["O_T_Boat_Armed_01_hmg_F"]]; // = 30
_veh set [T_VEH_personal, ["O_Quadbike_01_F"]]; // = 31
_veh set [T_VEH_truck_inf, ["O_T_Truck_03_transport_ghex_F", "O_T_Truck_03_covered_ghex_F", "O_T_Truck_02_transport_F", "O_T_Truck_02_F"]]; // = 32 – REQUIRED
_veh set [T_VEH_truck_cargo, ["O_T_Truck_03_transport_ghex_F", "O_T_Truck_03_covered_ghex_F"]]; // = 33
_veh set [T_VEH_truck_ammo, ["O_T_Truck_03_ammo_ghex_F", "O_T_Truck_02_Ammo_F"]]; // = 34 – REQUIRED
_veh set [T_VEH_truck_repair, ["O_T_Truck_02_Box_F", "O_T_Truck_03_repair_ghex_F"]]; // = 35
_veh set [T_VEH_truck_medical , ["O_T_Truck_02_Medical_F", "O_T_Truck_03_medical_ghex_F"]]; // = 36
_veh set [T_VEH_truck_fuel, ["O_T_Truck_02_fuel_F", "O_T_Truck_03_fuel_ghex_F"]]; // = 37
