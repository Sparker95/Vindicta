...
//Updated: August 2021 by Jasperdoit

_array = [];

_array set [T_SIZE-1, nil]; //Make an array having the size equal to the number of categories first

_array set [T_NAME, "tCiviliansmallguns"];                           //Template name + variable (not displayed)
_array set [T_DESCRIPTION, "Standard Altis civilians from base game. Primary weapons are removed in this faction."];     //Template display description
_array set [T_DISPLAY_NAME, "Arma 3 Altis Civilians (Pistol)"];      //Template display name
...
// _uc set[T_UC_civVehs, T_UC_civVehs];
_array set [T_UC, _uc];

//==== Infantry ====
_inf = [];
_inf resize T_INF_SIZE;
_inf = _inf apply { ["Arma3_CIVILIAN_Default_2"] };
