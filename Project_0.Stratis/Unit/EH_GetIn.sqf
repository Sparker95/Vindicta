...
diag_log format ["[Unit::EH_GetIn] Info: _this: %1, _unitVeh: %2, _unitInf: %3, typeOf _vehicle: %4", _this, _unitVeh, _unitInf, typeof _vehicle];
...
if (_garrison != "") then {	// Sanity check
	pr _args = [_unitVeh, _unitInf];
	CALLM2(_garrison, "postMethodAsync", "handleGetInVehicle", _args);
} else {
	diag_log format ["[Unit::EH_GetIn] Error: vehicle is not attached to a garrison: %1, %2", _unitVeh, _data];
};