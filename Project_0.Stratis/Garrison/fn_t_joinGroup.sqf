...
private _unitDatas = _requestData select 0;
private _newGroupID = _requestData select 1;
private _keepStructure = _requestData select 2;

//_unitDatas can be an array of [_catID, _subcatID, _unitID] or it can be a single [_catID, _subcatID, _unitID]
if((_unitDatas select 0) isEqualType 0) then
{
	_unitDatas = [_unitDatas];
};

for "_i" from 0 to ((count _unitDatas) - 1) do
{
	private _unitData = _unitDatas select _i;
	
	//Read the input
	_unitData params ["_catID", "_subcatID", "_unitID"];
