...
[[group cursorObject], getPos player, 20] spawn (compile preprocessfilelinenumbers "AI\casual.sqf");

_params:
	[_location, _maxRadius]
*/

params ["_groups", "_params"];

private _loc = _params select 0;
private _maxRadius = _params select 1;
...
private _chat =
{
	...
	(_men select 0) setVariable ["AI_hBS", nil];
	(_men select 1) setVariable ["AI_hBS", nil];
};
	...
	if(random 1 < 0.5) then
	{
		...
		while {_i > 0} do
		{
			if(_freeUnitsNr >= 2) then
			{
				...
				_man1 setVariable ["AI_hBS", _scriptHandle];
				_man2 setVariable ["AI_hBS", _scriptHandle];
			};
			_i = _i - 1;
		};
	}
			...
			//diag_log format ["Number of campfires: %1", count _campfires];
			{
				if(_freeUnitsNr > 0) then
				{
					...
					while {_menAtCampfireNr < _menAtCampfireMax && _freeUnitsNr > 0} do
					{
						...
						_campfireMan setVariable ["AI_hBS", _scriptHandle]; //Set the script handle for unit. It will be used later to terminate the spawned scripts.
...
//todo make AI spin the carousel
/*
{#line 1 "A3\Structures_F_EPC\Civ\PlayGround\scripts\Carousel_spin.sqf"
params["_obj","_phase"];

playSound3D ["A3\Sounds_F\structures\Carousel\Carousel_Turn_0" + (selectRandom ["1","2"] ) + ".wss",_obj,false,getposASL _obj,1,1,20];

_obj animate ['Spin_1_rot', _phase];
sleep 0.25;
_obj animate ['Spin_2_rot', _phase];
sleep 0.25;
_obj animate ['Spin_3_rot', _phase];

true}

player attachto [car, [0, -0.8, 0.4]];
player switchmove "passenger_bench_1_Idle";
