...
private _loc = objNull;
{
	...
	private _possibleTypes = ["base", "outpost","village","city","capital"];
	{
		if(_mrk find _x == 0)exitWith{
			_type = _x;
		};
	...
	if(!(_type isEqualTo "")) then {
		diag_log format ["[Location::createAllFromEditor] Info: Trying to create location type: %1",_type];
		...
		if(_type in ["village", "city","capital"])then{
			private _cpModule = [_mrkPos,_mrkSize] call CivPresence_fnc_init;
			SET_VAR(_loc, "cpModule",_cpModule);
			diag_log format ["[Location::createAllFromEditor] Info: Location: %1",_debugName];
	
		};	
		
		
		if(_type in ["base","outpost"])then{
